...
def fetch_data_from_bloomberg(ticker_symbols, start_date, end_date):
        ...
        if event.eventType() == blpapi.Event.RESPONSE or event.eventType() == blpapi.Event.PARTIAL_RESPONSE:
            for msg in event:
                ...
                for i in range(field_data.numValues()):
                    row = [ticker, field_data.getValue(i).getElement(1).getValue()]
                    for j in range(len(field_data.getValue(i).getElement(2).elements())):
                        row.append(field_data.getValue(i).getElement(2).getElement(j).getValue())
                    data.append(row)
    ...
    data_df = pd.DataFrame(data, columns=["Ticker", "Currency", start_date.strftime('%Y-%m-%d')])
